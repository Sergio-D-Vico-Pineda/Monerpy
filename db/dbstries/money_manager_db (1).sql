...
-- 2. Categories
-- Defines transaction categories for income and expenses
CREATE TABLE categories (
    id SERIAL PRIMARY KEY, -- unique identifier
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, -- owned by a specific user
    name VARCHAR(255) NOT NULL, -- category name, e.g., 'Groceries'
    type VARCHAR(50) CHECK (type IN ('income', 'expense')) -- distinguishes income vs expense
);
...
CREATE TABLE transactions (
    ...
    category_id INTEGER REFERENCES categories(id), -- classification
